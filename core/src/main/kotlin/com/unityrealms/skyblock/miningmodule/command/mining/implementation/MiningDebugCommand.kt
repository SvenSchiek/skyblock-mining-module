...
/** Shows runtime mining diagnostics. */
class MiningDebugCommand(private val permission: String) : Command {
    ...
    player.sendMessage(
      ComponentTransformer.transform(
        MiningCore.resolveMessage(
          "message.command.mining.debug",
          MiningCore.miningProfileManager.cachedProfileCount,
          MiningCore.miningSessionManager.activeSessionCount,
          MiningCore.miningRespawnManager.depletedBlockCount,
          MiningCore.miningBlockRegistry.size,
          MiningCore.mineRegistry.size
        )
      )
    )
    return true
