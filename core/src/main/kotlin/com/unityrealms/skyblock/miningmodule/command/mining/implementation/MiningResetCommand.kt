...
/** Resets a player's mining profile. */
class MiningResetCommand(private val permission: String) : Command {
  ...
  override fun execute(player: Player, argumentArray: Array<String>): Boolean {
    ...
    if (argumentArray.isEmpty()) {
      player.sendMessage(ComponentTransformer.transform(MiningCore.resolveMessage("message.command.mining.reset_usage")))
      return true
    }

    val target = CommandValidator.resolveOfflinePlayer(argumentArray[0], player) ?: return true
    MiningCore.miningProfileManager.reset(target.uniqueId)
    player.sendMessage(ComponentTransformer.transform(MiningCore.resolveMessage("message.command.mining.reset_success", target.name ?: argumentArray[0])))
    return true
  }
